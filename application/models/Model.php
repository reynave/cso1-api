...
class Model extends CI_Model
{
    ...
    {
        if ($name) {
            $number = $this->model->select('runningNumber', 'autoNumber', "name = '" . $name . "'") + 1;
            $prefix = $this->model->select('prefix', 'autoNumber', "name = '" . $name . "'");
            if ($prefix == '$year') {
                $prefix = date("Y");
            }
            $update = array(
            ...
            $new_number = str_pad($number, $this->model->select('digit', 'autoNumber', "name = '" . $name . "'"), "0", STR_PAD_LEFT);
